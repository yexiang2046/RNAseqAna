...
process COUNT_PEAK_READS {
    ...
    raw_depth_data <- tryCatch({
        data <- read.table("${bam_meta}_viral_depth.txt", 
                          header=FALSE,
                          col.names=c("chr", "pos", "depth"),
                          colClasses=c("character", "integer", "integer"))
        if(nrow(data) == 0) {
            stop("No depth data found")
        }
        data
    }, error = function(e) {
        cat("Error reading depth data:", e$message, "\n")
        # Create empty data frame with correct structure
        data.frame(chr=character(), pos=integer(), depth=integer())
    })
