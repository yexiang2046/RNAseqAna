...
process TRIM{
	...
	output:
	tuple	val(sample_id), path("${projectDir}/trimmed/${sample_id}1.fastp.fastq.gz"), path("${projectDir}/trimmed/${sample_id}*2.fastp.fastq.gz")

	script:
	"""
	fastp -w 16 -l 20 -i ${reads[0]} -I ${reads[1]} -o ${projectDir}/trimmed/${sample_id}1.fastp.fastq.gz -O ${projectDir}/trimmed/${sample_id}2.fastp.fastq.gz
	"""
}
