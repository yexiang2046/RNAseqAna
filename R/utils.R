...
plot_vol <- function(tb, log_FC, color_threshold = 0.002, title = "Comparison"){
  dat <- tb
  subdat <- subset(dat, padj < color_threshold & abs(logFC) > log_FC)
  p <- ggplot(data = dat, aes(x = logFC,
                              y = -log10(PValue), label = symbol,
                              colour = threshold))+
    geom_point(size = 0.5) +
    ...
    theme_cowplot() +
    geom_text_repel(data = subdat, aes(x = logFC, y = -log10(PValue), label = symbol, max.overlaps = 20))
  return(p)
}
