...
def extend_splice_sites(splice_site_file, num_of_base, out_bedfile):
    ...
    Five_pd_up = pd.DataFrame({"Start": positions.loc[:,"Five"].add(-num_of_base)}) 
    Five_pd_down = pd.DataFrame({"End": positions.loc[:,"Five"].add(numb_of_base)})
    
    Three_pd_up = pd.DataFrame({"Start": positions.loc[:,"Three"].add(-numb_of_base)}) 
    Three_pd_down = pd.DataFrame({"End": positions.loc[:,"Three"].add(numb_of_base)})
    ...
    Five_pd.drop_duplicates().to_csv(Five_prime_bed, sep="\t", header=False, index=False)
    Three_pd.drop_duplicates().to_csv(Three_prime_bed, sep="\t", header=False, index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('num_of_base',
                        type=int,
                        help='number of base to extend on both side')
...
extend_splice_sites(args.splice_site_file, args.num_of_base, args.output_bed)
